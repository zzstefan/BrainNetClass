function [x, funVal, ValueL]=mtLeastR_SSGSR(A, y, D, z, lambda2, opts)
...
if (opts.mFlag==0 && opts.lFlag==0)
    ...
    for iterStep=1:opts.maxIter
        ...
        % added
        funVal(iterStep)=funVal(iterStep)+ lambda2*trace(x*D*x');
...
%                         break;
%                     end
                    if (abs( (funVal(iterStep) - funVal(iterStep-1))/funVal(iterStep-1) ) <= opts.tol)
                        break;
                    end
                end
            case 1
